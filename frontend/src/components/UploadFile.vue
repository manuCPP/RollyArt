...
<img v-if="opera.imgPath != null"  :src="`http://localhost:8000/storage/${opera.imgPath}?t=${forceRefresh}`" />
</template>
...
const handleFileChange = (event) => {
  ...
  console.log('📁 File selezionato:', file.value)
  uploadFile()
}
...
const uploadFile = async () => {
  if (!file.value) return
    ...
    const titolo = ref("sono un titolo");
    const descrizione = ref("sono una descrizione");
